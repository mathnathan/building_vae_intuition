...
# Load Data
synthData = np.load("data/single_gaussians_sizes=3.npy")
synthDataLabels = np.load("data/single_gaussians_sizes=3_labels.npy")
n_samples, input_dim = synthData.shape
winSize = int(np.sqrt(input_dim)) # The window size

np.random.seed(920901)
tf.set_random_seed(156399)


def latency_dim2(overwrite=False):
    """Vanilla autoencoder. Gaussian prior and posterior, Bernoulli likelihood."""
    print("\nRunning VAE with two dimensional latency space...\n")

    latency_dim = 2
    encoder = [500]*2 # num neurons in each layer of encoder network
    decoder = [500]*2 # num neurons in each layer of generator network
...
def latency_dim1(overwrite=False):
    """Vanilla autoencoder. Gaussian prior and posterior, Bernoulli likelihood."""
    print("\nRunning VAE with two dimensional latency space...\n")
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(total_batch):
                ...
                if i%interval == 0:
                    ...
                    axes.set_xlim((-8,8))
                    #axes.set_ylim((-8,8))
                    test_xs = synthData[5000:20000]
                    test_ys = synthDataLabels[5000:20000]
                    mean, std = vae.getLatentParams(test_xs)
                    latent_xs = mean + std*np.random.normal(size=std.shape)
                    bins = np.linspace(-8,8,100)
                    x1 = latent_xs[test_ys==0]
                    axes.hist(x1, bins, alpha=0.5, label='small')
                    x2 = latent_xs[test_ys==1]
                    axes.hist(x2, bins, alpha=0.5, label='medium')
                    x3 = latent_xs[test_ys==2]
                    axes.hist(x3, bins, alpha=0.5, label='large')

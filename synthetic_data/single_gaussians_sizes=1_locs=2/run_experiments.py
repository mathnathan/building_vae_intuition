...
# Load Data
synthData = np.load("data/single_gaussians_sizes=1_locs=2.npy")
synthDataLabels = np.load("data/single_gaussians_sizes=1_locs=2_labels.npy")
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(total_batch):
                ...
                if i%interval == 0:
                    ...
                    x1 = latent_xs[test_ys.astype(bool)]
                    axes.hist(x1, bins, alpha=0.5, label='left')
                    x2 = latent_xs[np.logical_not(test_ys.astype(bool))]
                    axes.hist(x2, bins, alpha=0.5, label='right')

...
def create_dataset():
    ...
    var = np.array((15,15))
    means = [np.array((-7,0)), np.array((7,0))]
    for i in tqdm.tqdm(range(numelems)):
        index = np.random.choice(range(len(means)))
        mean = means[index] + np.random.normal()
        labels[i] = index
        dataset[i] = gauss2D(X, Y, mean=mean, var=var).flatten()
    np.save('single_gaussians_sizes=1_locs=2', dataset)
    np.save('single_gaussians_sizes=1_locs=2_labels', labels)
